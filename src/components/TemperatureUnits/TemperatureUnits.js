...
const TemperatureUnits = ({setUnits}) => {
    ...
    return (
        <div className="toggle-switch">
            <button className={tempUnit === 'metric' ? ' active' : ''} onClick={() => handleToggle('metric')}><sup>o</sup>C</button>
            <h2>|</h2>
            <button className={tempUnit === 'imperial' ? ' active' : ''} onClick={() => handleToggle('imperial')}><sup>o</sup>F</button>
        </div>
    )
}
